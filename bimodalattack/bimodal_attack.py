...
class BimodalAttack:
    ...
    def run(
        ...
        self.messages_template, self.prompt_template_str = self._prepare_message_template(messages)
        self.target_ids = self.tokenizer(target, add_special_tokens=False, return_tensors="pt")["input_ids"].to(self.model.device)
        ...
        image_original, best_image = (image.clone(), image.clone()) if config.pgd_attack else (None, None)
        ...
        for i in tqdm(range(config.num_steps), desc="Bimodal Attack"):
            ...
            if config.pgd_attack and config.pgd_after_gcg:
                ...
                pgd_start_time = time.perf_counter()
                image = self.perform_pgd_step(image, config.eps, config.alpha, image_grad_after, image_original)
                current_pgd_time += time.perf_counter() - pgd_start_time
            pgd_times.append(current_pgd_time)

            losses.append(current_loss)
            current_optim_str = self.tokenizer.decode(optim_ids.squeeze(0), skip_special_tokens=True)
            ...
            self._log_iteration_summary(i, current_loss, best_loss, current_optim_str, iter_total_time, grad_time, sampling_time, loss_time, current_pgd_time)
        ...
        return BimodalAttackResult(
            best_loss=best_loss, best_string=self.tokenizer.decode(best_optim_ids.squeeze(0), skip_special_tokens=True),
            losses=losses, strings=optim_strings, adversarial_suffixes=adv_suffixes, model_outputs=model_outputs, 
            gradient_times=gradient_times, sampling_times=sampling_times, loss_times=loss_times, pgd_times=pgd_times, total_times=total_times
        )

    def candidate_sampling(self, optim_ids: Tensor, optim_ids_onehot_grad: Optional[Tensor]) -> Tensor:
        if not self.config.gcg_attack:
            return optim_ids
        
        if optim_ids_onehot_grad is None:
            logger.warning("Received None for GCG gradient, skipping candidate sampling for this step.")
            return optim_ids
            
        sampled_ids = sample_ids_from_grad(
            optim_ids.squeeze(0), optim_ids_onehot_grad.squeeze(0),
            self.config.search_width, self.config.topk, self.config.n_replace,
    ...
    def _get_prompt_texts_for_candidates(self, optim_ids_batch: Tensor) -> List[str]:
        optim_strings = self.tokenizer.batch_decode(optim_ids_batch, skip_special_tokens=True)
        return [self.prompt_template_str.format(optim_str=s) for s in optim_strings]

    def _compute_candidates_loss(self, sampled_ids: Tensor, image: Optional[Tensor]) -> Tensor:
        all_losses = []
        ...
        for i in range(sampled_ids.shape[0]):
            candidate_ids = sampled_ids[i:i+1] 
            prompt_text = self._get_prompt_texts_for_candidates(candidate_ids)[0]
            ...
            if self.config.early_stop and torch.any(torch.all(torch.argmax(shift_logits, dim=-1) == shift_labels, dim=-1)):
                self.stop_flag = True
                break
    ...
    def compute_gradient(self, optim_ids: Tensor, image: Optional[Tensor]) -> Tuple[Optional[Tensor], Optional[Tensor]]:
        ...
        if len(anchor_indices) == 0:
            raise RuntimeError(f"Could not locate start anchor token '{self.start_anchor}' in prompt.")
        
        diff_part_start_idx = anchor_indices[0].item()
        
        placeholder_str = f"{self.start_anchor}{optim_str}{self.end_anchor}"
        placeholder_tokens_list = self.tokenizer(placeholder_str, add_special_tokens=False)['input_ids']
        placeholder_tokens = torch.tensor(placeholder_tokens_list, device=self.model.device).unsqueeze(0)
        ...
        one_hot = torch.nn.functional.one_hot(placeholder_tokens, num_classes=self.embedding_layer.num_embeddings).to(self.model.dtype)
        ...
        final_embeds = torch.cat([prefix_embeds, differentiable_part_embeds, postfix_embeds], dim=1)
        
        if self.config.pgd_attack:
            inputs['pixel_values'].requires_grad_()
            grad_targets.append(inputs['pixel_values'])
            
        outputs = self.model(inputs_embeds=final_embeds, attention_mask=inputs['attention_mask'])
        logits = outputs.logits
        ...
        if not grad_targets: return None, None
    ...
    def perform_pgd_step(self, image: Tensor, eps: float, alpha: float, image_grad: Optional[Tensor], image_original: Tensor) -> Tensor:
    ...
    def _log_iteration_summary(self, i, loss, best_loss, suffix, total_t, grad_t, sample_t, loss_t, pgd_t):
        summary_msg = (
